export const siteConfig = {
  name: "Adeyeye Emmanuel",
  title: "GoHighLevel Automation Specialist",
  tagline:
    "GoHighLevel Expert | Sales Funnel Builder | CRM Automation Specialist | Social Media Manager",
  headline:
    "Helping Businesses Automate Growth, Generate Leads, and Scale Revenue",
  description:
    "GoHighLevel Expert helping businesses automate operations, generate leads,  and scale revenue through CRM automation, funnels, and marketing systems.",
  email: "aemmanuel0002@gmail.com",
  phone: "+234 814 374 8235",
  whatsapp: "https://wa.me/2348143748235",
  calendly: "https://calendly.com/aemmanuel0002/30min",
  contactFormEndpoint:
    "https://www.fastsubmit.cloud/api/submit/nXGBU1nTxesDjhikVjfZ",
  location: "Remote",
  heroImage: "/hero-image.jpg",
  testimonialVideo: {
    title: "Client Video Testimonial",
    source: "google-drive" as const,
    fileId:
      "https://drive.google.com/file/d/1cfKHFMCrb4V-4L_j8gI5lsIfSPNcMI6g/view?usp=drive_link",
  },
  social: {
    linkedin: "https://www.linkedin.com/in/emmanuel-adeyeye-06a13b211",
    facebook: "https://www.facebook.com/profile.php?id=61588219356979",
    instagram: "https://www.instagram.com/editandautomate/",
    upwork:
      "https://www.upwork.com/freelancers/~011dac7f71625f4da2?mp_source=share",
  },
  stats: {
    funnels: 20,
    workflows: 100,
    campaigns: 75,
    platforms: 12,
  },
  navLinks: [
    { label: "About", href: "#about" },
    { label: "Skills", href: "#skills" },
    { label: "Services", href: "#services" },
    { label: "Portfolio", href: "#portfolio" },
    { label: "Process", href: "#process" },
    { label: "Contact", href: "#contact" },
  ],
} as const;
